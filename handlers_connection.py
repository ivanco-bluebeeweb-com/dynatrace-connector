"""Connection lifecycle for Dynatrace Connector."""
...
@chat.function(
    "connect_dynatrace",
    "Connect Dynatrace account via credentials.",
    action_type="write",
    chain_callable=True,
    event="dynatrace-connector.connect_dynatrace",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_dynatrace(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Dynatrace Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Dynatrace Account",
...
@chat.function(
    "list_connections",
    "List connected Dynatrace accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_dynatrace",
    "Disconnect Dynatrace account.",
    action_type="write",
    chain_callable=True,
    event="dynatrace-connector.disconnect_dynatrace",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_dynatrace(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
