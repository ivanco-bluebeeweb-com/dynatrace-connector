"""Panel UI for Dynatrace Connector following UI_INTERFACE_STANDARD.md and AUTH_AND_CREDENTIALS_STANDARD.md."""
...
def _settings_button() -> ui.UINode:
    return ui.Button(
        ...
        size="sm",
        icon="settings",
        on_click=ui.Call("__panel__dynatrace_settings")
    )

def _help_modal() -> ui.UINode:
    return ui.Modal(
        trigger=ui.Button("How do I set this up?", variant="ghost", size="sm"),
        title="Connecting Dynatrace",
        children=[
            ui.Text(
                "1. Sign in to your Dynatrace account and navigate to API/Integration or OAuth settings.\n2. Choose your preferred authentication method (OAuth SSO, API Key / Personal Token, or Client Credentials / Service Account).\n3. Authorize or enter your credentials above and click Connect.",
                variant="body"
            )
        ]
    )

@ext.panel("dynatrace_sidebar", slot="left")
async def dynatrace_sidebar(ctx, **kwargs) -> ui.UINode:
    return ui.Stack(
        direction="v",
        gap=3,
        align="stretch",
        children=[
            ui.Text("Dynatrace", variant="heading"),
            ui.Stack(
                direction="v",
                gap=1,
                align="stretch",
                children=[
                    ui.Text("Manage your Dynatrace connections and integrations.", variant="caption"),
                ]
            ),
            ui.Divider(),
            ui.Stack(
                ...
                children=[
                    ui.Button(
                        "Sign in with Dynatrace (OAuth / SSO)",
                        variant="primary",
                        size="sm",
                        icon="login"
                    ),
                    ...
                    ui.Form(
                        submit_label="Connect Dynatrace",
                        action=ui.Call("connect_dynatrace"),
                        children=[
                            ui.Stack(
                                direction="v",
                                gap=2,
                                ...
                                children=[
                                    ...
                                    ui.Stack(
                                        ...
                                        children=[
                                            ...
                                            ui.Input(param_name="label", placeholder="e.g. Production Dynatrace"),
                                        ]
                                    ),
                                    ui.Stack(
                                        direction="v",
