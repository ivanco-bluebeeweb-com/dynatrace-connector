# PST Testing Scenarios for Dynatrace Connector

## Part A: Authentication & Connectivity Verification
- **Scenario A1: Valid Credential Connect**: `connect_dynatrace_connector` saves token and validates endpoint.
...
- **Scenario B1: List Metrics**: `list_metrics` returns typed list with bounds.
- **Scenario B2: Get Metric**: `get_metric` returns entity details or 404 error.
- **Scenario B3: Health Audit**: `audit_metric_health` aggregates active records.

## Part C: Safe Write & Idempotency Testing
- **Scenario C1: Connection Lifecycle**: Clean disconnect via `disconnect_dynatrace_connector` without lingering secrets.
